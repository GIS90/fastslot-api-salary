...
APIs：
    > /access/login：登录
    > /access/logout：退出
    > /access/auth：用户菜单权限，用于系统登录后获取用户权限菜单树
"""
...
async def login(
    ...
    model: Dict = await user_service.login_by_rtx_id(rtx_id=username)
    # >>> 用户不存在
    if not model:
        return FailureStatus(

...
class XtbXtcsCurd(BaseCurd):
    ...
    async def _get_model_by_field(
        ...
        try:
            ...
            result = await db.execute(select(XtbXtcsModel).where(
                _field == value, XtbXtcsModel.status != 1)
            )
    ...
    async def get_by_id(self, db: AsyncSession, _id: int):
        return await self._get_model_by_field(db, XtbXtcsModel.id, _id)

    async def get_by_key(self, db: AsyncSession, key: str):
        return await self._get_model_by_field(db, XtbXtcsModel.key, key)

    async def get_by_md5(self, db: AsyncSession, md5: str):
        return await self._get_model_by_field(db, XtbXtcsModel.md5, md5)

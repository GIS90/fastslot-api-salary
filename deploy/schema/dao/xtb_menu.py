...
class XtbMenuModel(baseModel, IDField, Md5Field, CUDField, StatusField, OrderIdFiled):
    ...
    name: Mapped[str] = mapped_column(name="name", type_=String(55), nullable=False, comment="路由英文名称，大驼峰命名方式[注：需要与父节点连接映射]，例如SystemMenu")
    path: Mapped[str] = mapped_column(name="path", type_=String(255), nullable=False, comment="路由path，全小写字母[注：需要与父节点连接映射]")
    title: Mapped[str] = mapped_column(name="title", type_=String(35), nullable=False, comment="菜单标题")
    pid: Mapped[int] = mapped_column(name="pid", type_=Integer, nullable=False, comment="父ID")
    level: Mapped[int] = mapped_column(name="level", type_=Integer, nullable=False, default=1, comment="菜单级别，默认1级菜单，根节点为0")
    type: Mapped[str] = mapped_column(name="type", type_=String(35), nullable=False, default="MENU", comment="菜单类型：MENU=菜单，LINK=外链，BUTTON=按钮")
    component: Mapped[str] = mapped_column(name="component", type_=String(255), nullable=False, comment="路由组件，与Vue router mappings映射[注：需要与父节点连接映射]")

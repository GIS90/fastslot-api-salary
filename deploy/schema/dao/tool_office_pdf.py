...
class ToolOfficePdfModel(baseModel, IDField, Md5Field, RcUDField, StatusField):
    ...
    store_url: Mapped[str] = mapped_column(name="store_url", type_=String(130), comment="原始文件store对象存储资源路径（绝对路径）")
    transfer_url: Mapped[str] = mapped_column(name="transfer_url", type_=String(130), comment="转换文件store对象存储资源路径（绝对路径）")

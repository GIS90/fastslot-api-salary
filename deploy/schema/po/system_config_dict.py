...
__all__ = ["CsbEnumKeyAddModel", "CsbEnumKeyUpdateModel"]


class __CsbEnumKeyBaseModel(baseModel):
    remark: str = Field(..., min_length=1, max_length=35, description="枚举说明", alias="remark")
...
class CsbEnumKeyAddModel(__CsbEnumKeyBaseModel):
    key: str = Field(..., min_length=1, max_length=35, description="枚举KEY值（允许字母、数字、连字符(-)和点(.)的组合）", alias="key")
    ...
    def field_is_key(cls, value: str) -> str:
        return alphanumeric_only(value=value, field="枚举KEY值")
    # - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -

...
def md5(v: str, length: int = 32) -> str:
    """
    字符串md5加密
    ...
    if isinstance(v, str):
        v = v.encode('utf-8')
    if length >= 32: length = 32    # 配置最大长度
    v_md5 = hashlib.md5(v).hexdigest()
    return v_md5[0: length] if length < 32 else v_md5
...
def alphanumeric_only(value: str, field: str) -> str:
    if not re.match(r'^[a-zA-Z0-9\-\.]+$', value):
        raise RequestValidationError(f"{field}字段只能包含字母、数字、连字符(-)和点(.)")
    return value